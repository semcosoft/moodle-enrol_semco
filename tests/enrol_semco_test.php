...
final class enrol_semco_test extends \advanced_testcase {
    ...
    public function test_check_user_existence_by_field_successful($field, $value, $exists): void {
        ...
        $this->assertArrayHasKey('userid', $webservicereturn);
        $this->assertArrayHasKey('firstname', $webservicereturn);
        $this->assertArrayHasKey('lastname', $webservicereturn);
        $this->assertArrayHasKey('email', $webservicereturn);
        $this->assertArrayHasKey('username', $webservicereturn);
        $this->assertArrayHasKey('idnumber', $webservicereturn);

        // Check the webservice return value.
        $this->assertEquals($exists, $webservicereturn['userexists']);
        if ($exists) {
            $this->assertEquals($user->id, $webservicereturn['userid']);
            $this->assertEquals($user->firstname, $webservicereturn['firstname']);
            $this->assertEquals($user->lastname, $webservicereturn['lastname']);
            $this->assertEquals($user->email, $webservicereturn['email']);
            $this->assertEquals($user->username, $webservicereturn['username']);
            $this->assertEquals($user->idnumber, $webservicereturn['idnumber']);
        } else {
            $this->assertNull($webservicereturn['userid']);
            $this->assertNull($webservicereturn['firstname']);
            $this->assertNull($webservicereturn['lastname']);
            $this->assertNull($webservicereturn['email']);
            $this->assertNull($webservicereturn['username']);
            $this->assertNull($webservicereturn['idnumber']);
        }
    }
